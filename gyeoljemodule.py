...
class MainPage(Sharing):
    def __init__(self, master):
        ...
        if(PersonalCard.final_cost == 0):
            pay_btn = tk.Button(self.cart_frm, image=self.mv_pay_btn_img, relief="flat", bd=0,
                       command=lambda: master.switch_frame(OrderCheckPage), state = tk.DISABLED).place(x=400, y=0, width=80, height=100)
        else:
            pay_btn = tk.Button(self.cart_frm, image=self.mv_pay_btn_img, relief="flat", bd=0,
                          command=lambda: master.switch_frame(OrderCheckPage), state = tk.NORMAL).place(x=400, y=0, width=80, height=100)
...
class OrderCheckPage(Sharing):
    def __init__(self, master):
        ...
        # in announce frame #
        # 총액 정보란 #여기서 오더 디비 업데이트하면 되지 않을까용
    ...
    def show_orderlist(self):
        ...
        self.cur.execute("SELECT * FROM orderTable ORDER BY datetime(updateTime) ASC")
        rows = self.cur.fetchall()
        for menu in rows:
        ...
        def cc_menu(code):
            ...
            order_lbox.delete(0)
            if(menu[1] == "빅맥"):
                PersonalCard.final_cost -= 8000 * menu[2]
            elif(menu[1] == "맥너겟"):
                PersonalCard.final_cost -= 3000 * menu[2]
            else: 
                PersonalCard.final_cost -= 1500 * menu[2]  #쿠폰 쓴경우를 어떻게..
